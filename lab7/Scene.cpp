...
HRESULT Scene::init(ID3D11Device* device, ID3D11DeviceContext* context, int screenWidth, int screenHeight) {
    ...
    HRESULT hr = cube.init(device, context, screenWidth, screenHeight, { L"./koti.dds", L"./koti.dds"}, L"./texture_norm.dds", 32.f, cubePositions);
    if (FAILED(hr))
        return hr;

    std::vector<XMFLOAT4> planeColors = {
      XMFLOAT4(0.f, 1.f, 0.f, 0.5f),
      XMFLOAT4(1.f, 1.f, 0.f, 0.5f),
    };
    ...
    for (int i = 0; i < MAX_LIGHTS; i++) {
        colors[i] = XMFLOAT4(
            (float)(0.5f + rand() / (float)(RAND_MAX + 1) * 0.8f),
            (float)(0.5f + rand() / (float)(RAND_MAX + 1) * 0.8f),
            (float)(0.5f + rand() / (float)(RAND_MAX + 1) * 0.8f), 1.f);
